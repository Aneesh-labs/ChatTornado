...
export const ConversationCard = React.memo(({
    ...
    const canOpenChat = !isGlobal || connectionStatus === "accepted";
                    ...
                    <div className="flex items-center justify-end">
                        {connectionStatus === "accepted" ? (
                            ...
                            <button
                                ...
                                className="text-[10px] bg-violet-600 hover:bg-violet-500 text-white font-semibold px-2.5 py-1 rounded-full transition-colors active:scale-95"
                            >
                                Request
                            </button>
                        )}
...
const Sidebar = React.memo(({
    ...
    return (
        <div className={`w-full h-full flex flex-col relative ${theme?.sidebar || ""} border-r ${theme?.border || ""} backdrop-blur-2xl overflow-hidden`}>
            ...
            {/* Filter Tabs (All / Pinned / Unread) — Only shown in Chats tab */}
            {mainTab === "chats" && (
                <div className={`flex items-center gap-1.5 px-3 sm:px-4 py-2 border-b ${theme?.border || ""}`} role="tablist">
